defmodule OnElixir.CategoriesTest do
  use OnElixir.DataCase

  alias OnElixir.Categories

  describe "categories" do
    alias OnElixir.Categories.Category

    import OnElixir.CategoriesFixtures

    @invalid_attrs %{name: nil, parent_category_id: nil, posts_count: nil}
    ...
    test "create_category/1 with valid data creates a category" do
      valid_attrs = %{name: "some name", parent_category_id: "7488a646-e31f-11e4-aace-600308960662", posts_count: 42}

      assert {:ok, %Category{} = category} = Categories.create_category(valid_attrs)
      assert category.name == "some name"
      assert category.parent_category_id == "7488a646-e31f-11e4-aace-600308960662"
      assert category.posts_count == 42
    end
    ...
    test "update_category/2 with valid data updates the category" do
      category = category_fixture()
      update_attrs = %{name: "some updated name", parent_category_id: "7488a646-e31f-11e4-aace-600308960668", posts_count: 43}

      assert {:ok, %Category{} = category} = Categories.update_category(category, update_attrs)
      assert category.name == "some updated name"
      assert category.parent_category_id == "7488a646-e31f-11e4-aace-600308960668"
      assert category.posts_count == 43
    end
